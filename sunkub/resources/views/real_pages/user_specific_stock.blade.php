...
@section('content')
    <div class="cardliststock mx-8 mt-8 flex flex-col h-full px-4 ">
        ...
        <div class="flex flex-col justify-center mt-4 mx-3.5 bg-neutral-800 h-full pt-4 cardliststock">
            ...
            <div class="flex gap-4 px-56 h-18 my-16 justify-center">
                <form action="{{ route('presell', ['stock_symbol' => $stock->stock_symbol]) }}" method="GET" class="w-full h-full">
                    ...
                    <button type="submit" class="py-2.5 text-xl bg-red-400 hover:bg-red-700 w-full text-white text-center content-center rounded-xl">SELL</button>
                </form>
                <form action="{{ route('prebuy', ['stock_symbol' => $stock->stock_symbol]) }}" method="GET" class="h-full w-full">
                    @csrf
                    <input type="hidden" name="stock_symbol" value="{{ $stock->stock_symbol }}">
                    <button type="submit" class="py-2.5 text-xl bg-green-400 hover:bg-green-700 w-full text-white text-center content-center rounded-xl">BUY</button>
                </form>
            </div>
        </div>
    </div>
