...
@section('content')
    ...
    <div class=" h-full min-h-screen bg-neutral-700 rounded-t-2xl drop-shadow-lg  mx-16 mt-8 pt-4">
        ...
        <div class="grid grid-cols-2 gap-4 px-16">
            @foreach ($rankedResults as $brokerName => $stocks)
                <div class=" bg-neutral-800 rounded-xl p-4 ">
                    ...
                    <div class="text-sm mt-2 text-gray-200 opacity-50">แสดงหุ้น 3 อันดับ</div>
                    <div class="mt-8 px-20 mb-3 place-content-cente ">
                        @foreach ($stocks as $idx => $stock)
                            <div class="flex justify-between items-center px-3 mt-3">
                                <p class="text-lg {{ $fontColors[$idx % 3] }}">{{ $stock->stock_symbol }}</p>
                                <p class="text-lg {{ $fontColors[$idx % 3] }}">{{ $stock->total_volume }} หุ้น</p>
                            </div>
